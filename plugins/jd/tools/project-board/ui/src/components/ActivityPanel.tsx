      ...
      {jobs.map((job) => (
        <div key={job.id} className={`rounded-[10px] border bg-surface p-3 text-sm ${job.state === 'running' ? 'border-running-border shadow-[0_0_18px_rgba(67,217,232,.18)]' : 'border-border'}`}>
          <div className="flex items-center justify-between">
            ...
            <span className={`rounded-full border px-2 py-0.5 font-mono text-[12px] ${STATE_PILL[job.state]}`}>{job.state}</span>
          </div>
          {job.error && <p className="mt-1 text-xs text-danger">{job.error}</p>}
          {job.state === 'running' && (
            <p className="mt-2 line-clamp-2 font-mono text-[12px] text-text-muted">{previews[job.id] ?? 'Đang chờ output…'}</p>
          )}
